...
entrada = input("Digite a nota (ex: C4, A3, G5): ").strip().upper() #elimina espaco e deixa maiusculo

nota = entrada[0]
oitava = int(entrada[1])

if nota in base: #ate a quantidade da base 
    freq = base[nota]
    
    #freq * 2^(oitava - 4)
    final = freq * (2 ** (oitava - 4))
    
    print(f"A frequência de {entrada} é {final:.2f} Hz")
else:
    print("Nota inválida.")